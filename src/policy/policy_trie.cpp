...
void PolicyTrie::insert(const Policy& policy) {
    ...
    std::string db = policy.scope.database.value_or(policy::kWildcard);
...
std::vector<const Policy*> PolicyTrie::find_matching(
    ...
    std::vector<std::string> path_parts = {
        database.empty() ? policy::kWildcard : database,
        schema.empty() ? policy::kWildcard : schema,
        table.empty() ? policy::kWildcard : table
    };
...
void PolicyTrie::find_matching_recursive(
    ...
    if (current_part != policy::kWildcard) {
        const PolicyTrieNode* wildcard_child = node->get_child(policy::kWildcard);
        if (wildcard_child) {
